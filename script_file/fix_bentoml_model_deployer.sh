...
ZENML_FILE_bentoml_model_deployer="/opt/venv/lib64/python3.11/site-packages/zenml/integrations/bentoml/model_deployers/bentoml_model_deployer.py"

cat > "$ZENML_FILE_bentoml_model_deployer" << 'EOF'
...
EOF
echo "✅ Completed overwriting of file: $ZENML_FILE_bentoml_model_deployer"
# Finally, run the container's main command (if any)
exec "$@"
