...
ZENML_FILE_bentoml_deployer="/opt/venv/lib64/python3.11/site-packages/zenml/integrations/bentoml/steps/bentoml_deployer.py"

cat > "$ZENML_FILE_bentoml_deployer" << 'EOF'
...
def bentoml_model_deployer_step(
    ...
    logger.info(
        f"BDH-model deployment service started and reachable at:\n"
        f"    {new_service.prediction_url}\n"
    )

    return new_service
EOF
echo "✅ Completed overwriting of file: $ZENML_FILE_bentoml_deployer"
